...
| 3 | `03_jwst_nircam_hip65426.ipynb` | JWST/NIRCam F444W HIP 65426 + reference star (MAST; `tutorials/fetch_jwst_hip65426.py`) | the spaceKLIP / pyKLIP path: minimal stage-2 cleaning and alignment, per-roll partitions, the reference library, and **searching a backend option** (pyKLIP's `mode`) — the optimizer picks RDI over ADI+RDI and takes HIP 65426 b from S/N 7.8 to 9.0 |
