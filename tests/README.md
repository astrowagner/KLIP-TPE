...
| `test_miri.py` | MIRI coronagraphic modes and published wavelengths, `default_azimuths`/`default_separations` (dense on the boundaries given, reaching the field edge), 2-D throughput interpolation including the 360-degree seam, `quadrant_mask` is a cross rather than a disc and follows a rotated mask, Lyot spot and support bar, `forbidden_pa` in sky angle over a whole roll sequence (and its two-roll floating-point tie), the injector attenuating each frame by its own throughput while a radial model still gets one for the sequence, `typical_throughput` not warning, `load_miri`/`apply_quadrant_mask` masking dead zones with NaN, `model_for_datasets` routing MIRI to the 2-D model and NIRCam to the radial one; one STPSF-backed check that the real map has the four-quadrant structure |
...
| `test_run_miri.py` | `scripts/run_miri.py` end to end on synthetic `calints` written by the test: the 2-D model reaching the reducer, dead zones NaN'd out of the cubes, injections barred from the dead sectors, `--no-mask-quadrants`, a NIRCam sequence refused rather than reduced with MIRI geometry, and `--check` refusing a radial throughput on a four-quadrant mask |
