...
| `test_miri.py` | MIRI coronagraphic modes and published wavelengths, `default_azimuths` dense on the boundaries it is given, 2-D throughput interpolation including the 360-degree seam, `quadrant_mask` is a cross rather than a disc and follows a rotated mask, Lyot spot and support bar, `MIRILibraryPSF` warning when asked for throughput without an azimuth, `load_miri` masking dead zones with NaN; one STPSF-backed check that the real map has the four-quadrant structure |
